...
`just check` runs formatting, tests, version-tool tests, and warnings-denied Clippy. `just package` builds both macOS slices, creates a Universal 2 binary, verifies its architectures and ad-hoc signature, and writes a deterministic archive plus checksum under `target/release/`. Native Windows CI runs the corresponding checks and release build on `x86_64-pc-windows-msvc`, then creates and inspects the deterministic Windows ZIP and checksum. Its native smoke checks absent, registered, and removed task state, then verifies deferred deletion of a copied release executable. Before release, inspect both archive member lists and checksums, the Universal 2 architectures and strict ad-hoc signature, and the Windows x64 fixture.
