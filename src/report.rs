...
use crate::{
    ...
    progress::Progress,
    rollout::{
        analysis::{AnalysisError, RolloutStats, TurnEvent, UsageEvent, replay_prefix_len},
    ...
    fn build_scope(
        ...
        for id in ids {
            ...
            let replay_prefix_len = record
                .parent_id
                .as_deref()
                .and_then(|parent_id| self.index.record(parent_id))
                .map_or(0, |parent| replay_prefix_len(parent, record));
            match self.cache.analyze_with_replay(record, replay_prefix_len) {
                Ok(stats) => {
...
mod tests {
    ...
    fn excludes_copied_fork_usage_and_keeps_the_first_child_request() {
        ...
        let filter = crate::date_filter::Filter::try_from(crate::date_filter::Options {
            since: None,
            through: None,
            group_by: Vec::new(),
            include_empty: false,
