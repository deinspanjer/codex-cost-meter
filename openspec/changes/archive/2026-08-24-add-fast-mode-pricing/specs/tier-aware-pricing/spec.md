...
The system SHALL retain each model's aggregate row and SHALL break its token usage and cost into Standard, assumed Standard, Fast, and unavailable-tier detail. Human output SHALL label Fast detail with `⚡` and assumed usage as `Standard (assumed)`; structured output SHALL identify each tier with a stable text key. Because tier is recorded per usage event rather than per turn, model turn count and duration SHALL remain on the aggregate model row and SHALL NOT be guessed or duplicated across tier detail rows.

#### Scenario: One model uses Standard and Fast
- **WHEN** a model has priced Standard and Fast usage events
- **THEN** its aggregate model report contains separate Standard and Fast token and cost detail whose sums equal the aggregate usage and known cost
- **AND** the human Fast row contains `⚡`

#### Scenario: One model has unavailable tier evidence
- **WHEN** a model has post-release usage before any applied-tier snapshot
- **THEN** its model report contains an assumed-Standard detail row with Standard cost and the report identifies the assumed token count

#### Scenario: One model has an unsupported explicit tier
- **WHEN** a model has usage after an unsupported tier value
- **THEN** its model report contains a corresponding unavailable-tier detail row with zero known cost and an incomplete estimate
