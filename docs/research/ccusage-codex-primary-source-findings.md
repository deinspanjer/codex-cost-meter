...
1. **A narrow replay invariant is confirmed for legacy explicit forks.** Codex source shows that legacy forks persist historical `token_count` records without making API requests for those records, while separately submitting inherited model-visible history in the first genuine child request. A 2026-08-24 read-only corpus audit found deterministic copied prefixes in 381 of 467 explicit forks; all were legacy history mode. Suppress only that structurally proven prefix and retain the first mismatch and every later child request.
...
The minimum useful follow-up is therefore: apply the confirmed explicit-legacy-fork correction; correct the published long-context pricing gap; and add date-bucketed reporting. Decide separately whether the product reports standard-price equivalents or recorded Fast/Priority rates. Do not copy ccusage's heuristic replay deletion, session-agnostic event signature, fuzzy price substitution, or silent zero-cost behavior unchanged.
...
| Fork/subagent replay | Exact parent-prefix suppression plus a fallback burst heuristic | No cross-rollout replay filter | Explicit legacy-fork overcount is proven; ccusage's broader heuristics remain unsafe |
...
After replay filtering, daily/monthly/weekly aggregation drops duplicate events with the same parsed timestamp, resolved-model hash and length, input, cached input, output, reasoning output, and total token counts. Session identity, request/source provenance, and service tier are omitted outside the session report ([dedupe key](https://github.com/ccusage/ccusage/blob/bd7f89b469aee5635fb2e6722dd6d70f2d113ac1/rust/adapters/codex/src/aggregate.rs#L488-L553)). If duplicate copies disagree about service tier, explicit metadata is retained and Standard wins a Standard/Fast conflict ([tier merge](https://github.com/ccusage/ccusage/blob/bd7f89b469aee5635fb2e6722dd6d70f2d113ac1/rust/adapters/codex/src/types.rs#L20-L35)).
...
What it lacks is cross-rollout replay recognition. Codex source history confirms that legacy forks physically persist historical `token_count` records with rewritten timestamps and seed the child cumulative baseline from the last copied record. Those records are accounting/UI restoration, not API requests. Model-visible inherited messages are reconstructed separately and submitted as the child request input; the server-reported cached and uncached input for that request remains genuine usage. Normalizing the parent and child independently therefore double-counts the copied prefix while correctly counting the later child delta. ccusage's timing fallback and global event key go farther than this proven invariant and retain undercount risk.
...
This is not a CLI-to-CLI benchmark because `codex-cost-meter` has no date-window command, and the probe did not independently label how much ccusage removed through exact parent matching versus its fallback heuristics or other parser-policy differences. It remains evidence of a material aggregate difference, not a decomposition of ccusage's removals.

### 2026-08-24 replay invariant audit

A separate privacy-safe audit selected 4,637 rollouts and resolved all 2,391 linked children to available parents. Of 467 explicit `forked_from_id` children, 266 copied the complete parent usage prefix and 115 copied a partial leading prefix. Every deterministic case used legacy history mode; no deterministic prefix appeared on paginated, ordinary subagent, or legacy parent-only edges. The partial cases each omitted exactly one recent parent record, consistent with the parent writing after the fork snapshot but before the child metadata timestamp. Suppression must therefore stop at the first mismatch rather than require the child to contain every parent record preceding its timestamp.

Mirroring the current parser's attribution and cumulative-delta rules, the 381 deterministic prefixes add 8,751,657,049 known duplicated total tokens: 8,723,852,504 gross input, including 8,376,620,800 cached input, plus 27,804,545 output tokens. Reasoning accounts for 9,192,173 of those output tokens and is not additive. Another 842,239,537 copied tokens become unattributed/incomplete rather than known usage. The first genuine post-prefix child requests total about 31.7 million tokens and must remain counted.

The audit also found 100 timing-only candidates containing about 7.53 billion total tokens. Their structure did not prove replay, so the one-second burst heuristic would create an unacceptable undercount risk. The accepted invariant requires explicit fork lineage, legacy copied-history structure, a resolvable parent, and exact leading token-component equality; it ignores rewritten timestamps, stops at the first mismatch, and retains every unmatched record.
...
1. **Apply only the confirmed explicit-legacy-fork invariant.** Recognize `forked_from_id`, require copied legacy history and a resolvable parent, remove the exact leading copied prefix, and stop at the first mismatch. Preserve every subsequent child request's input and cache usage. Do not extend suppression to ordinary subagent or legacy parent-only metadata without new source and corpus evidence.
